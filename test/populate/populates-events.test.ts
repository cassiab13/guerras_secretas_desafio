...
describe('Populates', () => {
    ...
    describe('POST /populates/:id', () => {
        ...
        it('should populate a database using Series id populate events', async () => {

            fetchStub.withArgs(createUrl(event.data.results[0].comics.collectionURI)).resolves({ json: () => Promise.resolve(comicComic)});
            fetchStub.withArgs(createUrl(event.data.results[0].creators.collectionURI)).resolves({ json: () => Promise.resolve(creatorComic)});
            fetchStub.withArgs(createUrl(event.data.results[0].stories.collectionURI)).resolves({ json: () => Promise.resolve(storieComic)});
            fetchStub.withArgs(createUrl(event.data.results[0].series.collectionURI)).resolves({ json: () => Promise.resolve(serieComic)});
            fetchStub.withArgs(createUrl(event.data.results[0].characters.collectionURI)).resolves({ json: () => Promise.resolve(characterComic)});

            const token: string = await getToken('user1', '1234');
            const response = await request.post(`/populates/${serieId}?events=true`).set('Authorization', `Bearer ${token}`);
            await new Promise(resolve => setTimeout(resolve, 2000));

            expect(response.status).toEqual(StatusCode.CREATED);

            const events: Event[] = await eventRepository.findAll();
            expect(events[0].title).toBe('Secret Wars');
            expect((events[0].comics as Comic[]).length).toBeGreaterThan(0);
            expect((events[0].series as Serie[]).length).toBeGreaterThan(0);
            expect((events[0].characters as Character[]).length).toBeGreaterThan(0);
            expect((events[0].creators as Creator[]).length).toBeGreaterThan(0);
            expect((events[0].stories as Storie[]).length).toBeGreaterThan(0);

            const populate: Populate | null = await populateRepository.findByIdSerie(parseInt(serieId));
            expect(populate?.events).toBeTruthy();
