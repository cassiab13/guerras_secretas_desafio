...
describe('Populates', () => {
    ...
    const characterRepository: CharacterRepository = new CharacterRepository(characterSchema);
    ...
    describe('POST /populates/:id', () => {
        it('should populate a database using Series id populate characters', async () => {

            fetchStub.withArgs(createUrl(character.data.results[0].comics.collectionURI)).resolves({ json: () => Promise.resolve(comicComic)});
            fetchStub.withArgs(createUrl(character.data.results[0].stories.collectionURI)).resolves({ json: () => Promise.resolve(storieComic)});
            fetchStub.withArgs(createUrl(character.data.results[0].events.collectionURI)).resolves({ json: () => Promise.resolve(eventComic)});
            fetchStub.withArgs(createUrl(character.data.results[0].series.collectionURI)).resolves({ json: () => Promise.resolve(serieComic)});

            const token: string = await getToken('user1', '1234');
            const response = await request.post(`/populates/${serieId}?characters=true`).set('Authorization', `Bearer ${token}`);
            await new Promise(resolve => setTimeout(resolve, 2000));

            expect(response.status).toEqual(StatusCode.CREATED);

            const characters: Character[] = await characterRepository.findAll();
            expect(characters[0].name).toBe('Absorbing Man');
            expect((characters[0].events as Event[]).length).toBeGreaterThan(0);
            expect((characters[0].comics as Comic[]).length).toBeGreaterThan(0);
            expect((characters[0].series as Serie[]).length).toBeGreaterThan(0);
            expect((characters[0].stories as Storie[]).length).toBeGreaterThan(0);

            const populate: Populate | null = await populateRepository.findByIdSerie(parseInt(serieId));
            expect(populate?.characters).toBeTruthy();
